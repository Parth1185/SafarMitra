...
class Prettify {
  BetweenStation(string) {
    try {
      ...
      // Error handling for no trains / invalid station
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (data1.length === 2) {
          ...
          // Add available classes (SL, 3A, 2A, 1A, CC, etc.)
          // Classes are usually at index 15 (check erail data)
...
obj["classes"] = data1[15]
  ...
  getDayOnDate(DD, MM, YYYY) {
    let date = new Date(YYYY, MM - 1, DD); // Month is 0-indexed
